#!/usr/bin/env python3
"""Compose or finalize hand-drawn poster artwork at deterministic dimensions.
...
try:
    ...
    sys.exit("Pillow is required. Try: python3 -m pip install Pillow")
...
def canvas_size(layout, ratio, width, exact_size, source_size):
    if exact_size is not None:
        ...
        raise ValueError("no default ratio is imposed; provide --source or specify --size/--canvas")

    if layout == "top-bottom" and ch % 2:
        if exact_size is not None:
            raise ValueError(f"top-bottom requires an even output height for exact 50/50, got {cw}x{ch}")
        ch += 1
    elif layout == "left-right" and cw % 2:
        if exact_size is not None:
            raise ValueError(f"left-right requires an even output width for exact 50/50, got {cw}x{ch}")
        cw += 1
...
def overflow_report(img, size, label):
    tw, th = size
    ...
    if src > dst:
        kept = dst / src
        message = f"  {label}: aspect {src:.3f} vs {dst:.3f} — source is wider, keeping {kept:.0%} of its width"
    else:
        kept = src / dst
        message = f"  {label}: aspect {src:.3f} vs {dst:.3f} — source is taller, keeping {kept:.0%} of its height"
    if kept < 0.75:
        message += "  << extend the background instead of cropping this hard"
    return message


def plan(layout, canvas, ratio_label):
    cw, ch = canvas
    ...
    if layout == "top-bottom":
        ...
        print(f"generate source and design at {pw}x{ph}  (aspect {pw / ph:.3f} = {pw // common}:{ph // common})")
        ...
        print("seam     none")
        print(f"generate the designed image at {pw}x{ph}  (aspect {pw / ph:.3f} = {pw // common}:{ph // common})")
        print("source   used as reference only; not visible in the output")


def axis_scores(img, layout):
    gray = img.convert("L")
    w, h = gray.size
    if layout == "top-bottom":
        small = gray.resize((min(w, 256), h), Image.LANCZOS)
        px = small.load()
        scores = [sum(abs(px[x, y] - px[x, y - 1]) for x in range(small.width)) / small.width for y in range(1, h)]
        return scores, h
    small = gray.resize((w, min(h, 256)), Image.LANCZOS)
    px = small.load()
    scores = [sum(abs(px[x, y] - px[x - 1, y]) for y in range(small.height)) / small.height for x in range(1, w)]
    return scores, w


def audit(path, layout, expected_size):
    img = load(path)
    w, h = img.size
    ...
    if expected_size is not None:
        if (w, h) == expected_size:
            ...
            print(f"size       OFF — requested {expected_size[0]}x{expected_size[1]}, got {w}x{h}")
    ...
    print(f"seam found {axis} = {seam}   (axis delta {strength:.1f}, page mean {mean:.1f})")
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Compose top-bottom, left-right, or design-only panel artwork.")
    p.add_argument("--layout", choices=("top-bottom", "left-right", "design-only"), default="top-bottom")
    p.add_argument("--source", "--top", dest="source", type=Path)
    p.add_argument("--design", "--bottom", dest="design", type=Path)
    p.add_argument("--out", type=Path)
    p.add_argument("--canvas")
    p.add_argument("--size", type=parse_size)
    p.add_argument("--width", type=int)
    p.add_argument("--anchor", choices=("top", "center", "bottom", "left", "right"), default="center")
    ...
    if adaptive and design_src.size != (pw, ph):
        ...
        if abs(source_aspect - design_aspect) / source_aspect > 0.005:
            sys.exit(f"source-adaptive mode requires design aspect {pw}:{ph}, got {design_src.width}:{design_src.height}; regenerate without stock-ratio cropping")
        print(f"adaptive design resize {design_src.width}x{design_src.height} -> {pw}x{ph} (same aspect, no crop)")
    adaptive_design = design_src.copy() if design_src.size == (pw, ph) else design_src.resize((pw, ph), Image.LANCZOS)
    if args.layout == "design-only":
        ...
        print(f"layout {args.layout}, canvas {cw}x{ch}, each panel {pw}x{ph}, seam at {axis}={seam}")
